...
const HomePage = () => {
    ...
    const [selectedTemplate, setSelectedTemplate] = useState<PaginationStateItem | undefined>(undefined);
    ...
    useEffect(() => {
        ...
        setUpdatedSeedData((prev) => ({
            ...prev,
            backgroundImages:
                scrappedData.image_urls.length > 0 ? scrappedData.image_urls?.map((scrapeImg) => BaseURL + scrapeImg) : prev.backgroundImages,
            ...
            texts:
                scrappedData.generated_titles.length > 0
                    ? scrappedData.generated_titles?.map((scrapeTitles) => scrapeTitles.title.toUpperCase())
                    : prev.texts,
        }));
    }, [scrappedData]);
