...
import { Typography, Box, IconButton, Stack, Alert, FormControlLabel, Checkbox } from '@mui/material';
...
import { createImage, createImageLogo, updateImageSource } from '../../utils/ImageHandler';
...
import { createBubbleElement, createBubbleElement1, updateBubbleElement } from '../../utils/BubbleHandler';
...
const Canvas: React.FC<CanvasProps> = React.memo(({ updatedSeedData, template }) => {
    ...
    const { paginationState, selectedPage, setSelectedPage, addPage, update } = usePaginationContext();
    ...
    const [overlayTextFiltersState1, setOverlayTextFiltersState1] = useState<FilterState>({
        color: '#909AE9',
    });
    ...
    const handleSelectionUpdated = () => {
        ...
        if (activeObject && activeObject.type === 'textbox') {
            ...
            if (activeObject.text.includes(selectedText)) {
                ...
                activeObject.setSelectionStyles({ textBackgroundColor: backgroundColor }, startPos, endPos);
    ...
    const removeBackgroundColor = () => {
        ...
        if (activeObject && activeObject.type === 'textbox') {
            ...
            if (activeObject.text.includes(selectedText)) {
                const startPos = activeObject.text.indexOf(selectedText);
                const endPos = startPos + selectedText.length;
                activeObject.setSelectionStyles({ textBackgroundColor: 'transparent' }, startPos, endPos);
                setBgColorApplied(true);
                canvasInstanceRef.current.renderAll(); // Render only the canvas instance
            }
        }
    ...
    const loadCanvas = useCallback(
        async (pageNumber?: string) => {
            const templateFound = paginationState?.find((item) => item.page === selectedPage);
    ...
    const updateBubbleImageContrast = () => {
        ...
        if (activeObject && activeObject.type === 'image') {
            ...
            activeObject.filters.push(new fabric.Image.filters.Contrast({ contrast }));
    ...
    const updateBubbleImageBrightness = () => {
        ...
        if (activeObject && activeObject.type === 'image') {
            ...
            activeObject.filters = []; // Clear existing filters
            activeObject.filters.push(new fabric.Image.filters.Brightness({ brightness }));
            activeObject.applyFilters();
        ...
        for (const customType of bgImages) {
            ...
            if (existingObject) {
                ...
                if (type === 'sharpen') {
                    ...
                    const sharpenIndex = existingObject.filters?.findIndex((fil) => fil instanceof fabric.Image.filters.Convolute);
                    if (sharpenIndex !== -1) {
    ...
    const [selectedColor, setSelectedColor] = useState(userMetaData?.company?.color || '#ffffff');
    ...
    const updateBackgroundImage = debounce((imageUrl: string) => {
        ...
        if (!activeObject) {
            if (template?.diptych === 'horizontal') {
                ...
                } else if (currentImageIndex !== undefined && currentImageIndex % 2 !== 0) {
    ...
    const findHighestPageNumber = (paginationState: PaginationStateItem[]): number => {
    ...
    const addTemplate = async () => {
        ...
        const templateFound = templateData.templates?.find((item) => item.filePath === paginationState[0].filePath);

        let templateJSON;

        try {
            templateJSON = await import(`../../constants/templates/${template.filePath}.json`);
        } catch (error) {
    ...
    const [hexConversionForElement, setHexConversionForElement] = useState<string | null>(null);
        ...
        >
            ...
            /> */}
                ...
                {activeTab == 'background' && dropDown && (
                    ...
                    >
                        <Paper className={classes.root}>
                            <div className={classes.optionsContainer} style={{ display: 'flex', justifyContent: 'space-evenly' }}>
                                <Button
                            ...
                            {activeButton === 'Filters' && (
                                <div className={classes.sliderContainer}>
                                    {availableFilters.map((filter) => (
                                        <Button
                                            key={filter.name}
                                            className={`${classes.button} ${selectedFilter === filter.name && classes.buttonActive}`}
                                            variant="text"
                                            color="primary"
                    ...
                    >
                        <Paper className={classes.root}>
                            ...
                            >
                                ...
                                <Typography className={classes.heading} onClick={() => setShow('fontWeight')}>
                                    FONTWEIGHT
                                </Typography>
                                <Typography className={classes.heading} onClick={() => setShow('charSpacing')}>
                                    SPACING
                                </Typography>
                                <Typography className={classes.heading} onClick={() => setShow('colors')}>
                                    COLORS
                                </Typography>
                                ...
                                {activeTab === 'element' && (
                                    <>
                                        <Typography className={classes.heading} onClick={() => setShow('opacity')}>
                                            OPACITY
                                        </Typography>
                                        <Typography className={classes.heading} onClick={() => setShow('element-shadow')}>
                                            SHADOW
                                        </Typography>
                                    </>
                                )}
                            ...
                            {show === 'element-shadow' && (
                                <div>
                                    ...
                                    <Box className={classes.optionsContainer}>
                                        ...
                                        <CustomColorPicker
                                            value={hexConversionForElement ? overlayTextFiltersState.color : 'rgba(0,0,0,1)'}
                                            changeHandler={(color: string) => {
                                    ...
                                    <Slider
                                        ...
                                        onChange={(event, newValue) => {
                                            ...
                                            const splitHexConvertion = hexConversionForElement.split(',');
                    ...
                    >
                        <Paper className={classes.root}>
                            ...
                            >
                                ...
                                <Typography className={classes.heading} onClick={() => setShow('shadow')}>
                                    SHADOW
                                </Typography>
                                <Typography
                                    className={classes.heading}
                                ...
                                <div>
                                    <div className={classes.colorPicker}>
                                        <Box className={classes.optionsContainer}>
                                            ...
                                            <CustomColorPicker
                                                value={hexConvertion ? overlayTextFiltersState.color : 'rgba(0,0,0,1)'}
                                                changeHandler={(color: string) => {
                                                    const rgbaColorCode = hexToRgbA(color);
                ...
                >
                    ...
                    <button onClick={() => updateActiveTab('element')} style={{ backgroundColor: 'transparent', border: 'none' }}>
                        <img
                    ...
                    <button onClick={() => updateActiveTab('writePost')} style={{ backgroundColor: 'transparent', border: 'none' }}>
                        <img
                            ...
                            <Box {...styles.uploadBox}>
                                <label style={styles.uploadLabel}>
                                    ...
                                    <form method="post" encType="multipart/form-data">
                                        <input type="file" onChange={(event) => uploadImage(event, 'backgroundImages')} style={{ display: 'none' }} />
                                    </form>
                        ...
                        <div>
                            ...
                            <Box {...styles.uploadBox}>
                                <label style={styles.uploadLabel}>
                                    ...
                                    <form method="post" encType="multipart/form-data">
                                        <input type="file" onChange={(event) => uploadImage(event, 'bubbles')} style={{ display: 'none' }} />
                                    </form>
                                    <IconButton color="primary" component="span">
                                        <CloudUploadIcon style={{ fontSize: '40px' }} />
                                    </IconButton>
                                        ...
                                        {swipeData?.map(({ swipeImg, id }, i) => {
                                            return (
                                                ...
                                                >
                                                    <img
                                                        src={swipeImg}
                                                        onClick={() => swipeColorChangeHandler(id, swipeImg)}
                                                        alt=""
                                                        style={{
                                                            cursor: 'pointer',
                                    ...
                                    >
                                            ...
                                            return (
                                                <img
                                                    ...
                                                    onClick={() => {
                                                        ...
                                                        const color = userMetaData?.company?.color || '#ffffff';
                                        ...
                                        <CustomColorPicker
                                            ...
                                            changeHandler={(color: string) => {
                                                ...
                                                if (canvas?._activeObject && canvas?._activeObject?.type === 'image')
                                                    existingObject = canvas?._activeObject as fabric.Image;

                                                if (!existingObject) {
                                                    console.log('existing Border object not founded');
                                                    return;
                                                }
                                                const blendColorFilter1 = new fabric.Image.filters.BlendColor({
                                                    color,
                                                    mode: 'tint',
                                                    alpha: 1,
                                                });
                                        ...
                                        {dividersData?.map(({ dividerImg }, i) => {
                                            return (
                                                ...
                                                >
                                                    <img
                                                        src={dividerImg}
                                                        onClick={() => dividerColorChangeHandler(dividerImg)}
                                                        // onClick={() => {
                                                        // 	fabric.Image.fromURL(
                                                        // 		dividerImg,
                                                        // 		function (img) {
                                ...
                                <Box>
                                    ...
                                    >
                                        {logos?.map((logo: string, i) => {
                                            const logoFillColor = userMetaData?.company?.color || 'black';
                                            return (
                                                <img
                                                    key={i}
                                                    src={logo}
                                        ...
                                        <CustomColorPicker
                                            ...
                                            changeHandler={(color: string) => {
                                                ...
                                                if (canvas?._activeObject && canvas?._activeObject?.type === 'textbox')
                                                    existingTextObject = canvas?._activeObject as fabric.Textbox;
